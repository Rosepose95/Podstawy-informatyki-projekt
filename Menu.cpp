	...
	, autoSlotText(arial)
{
    ...
    autoSlotText.setCharacterSize(28);          // TAKA SAMA WIELKOŚĆ JAK SLOTY
    autoSlotText.setFillColor(sf::Color::Black);

    // RĘCZNA korekta (JEDYNE MOŻLIWE ROZWIĄZANIE)
    autoSlotText.setPosition({
        autoSlotButton.getPosition().x - 140.f,  // lewo/prawo
        autoSlotButton.getPosition().y - 20.f   // góra/dół
        });
    float slotStartY =
...
void Menu::initSlots(sf::Font& font)
{
    ...
    float startY = autoSlotBottom + SLOT_SPACING; // slot 1 startuje poniżej autosave
...
void Menu::setSlotText(int slot, const std::string& text)
{
    ...
    // przeliczanie origin i ustawienie pozycji na środku slotu
